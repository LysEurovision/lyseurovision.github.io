...
export default function EventCard({event}: { event: Event }) {
    ...
    return (
        <LiveBorder active={event.live}>
            <div className="relative w-full rounded-xl bg-background dark:bg-neutral-900 border-1 border-foreground/25 dark:border-foreground/10">
                            ...
                            className={clsx('shrink-0 flex items-center cursor-pointer px-5 rounded-r-xl',
                                {
                                    ...
                                    'bg-sky-500 text-black': hasLinks() && !event.live,
                                    'bg-red-500 text-black outline outline-red-500': hasLinks() && event.live,
                                }
                            )}
                            ...
                            return <>{links.map((link, index) =>
                                <Link href={link.link} key={`${event.id}-link${index}`}>
                                    <div className={clsx('flex items-center gap-2 cursor-pointer px-2 py-3',
                                        {
                                            'bg-sky-500 text-black': ((!event.past && link.comment === 'Recommended link') || (event.past && index === 0)) && !event.live,
                                            'bg-red-500 text-black': ((!event.past && link.comment === 'Recommended link') || (event.past && index === 0)) && event.live,
...
function LiveBorder({active, children}: {active: boolean, children: React.ReactNode}) {
    return (
        <div className={clsx('relative w-full rounded-xl animate-[shimmer-rotate_4s_linear_infinite] bg-[conic-gradient(from_var(--border-angle),transparent_70%,var(--color-red-800)_85%,var(--color-red-500)_95%,transparent_100%)]', { 'p-0.5': active })}>
            { children }
        </div>
    );
}
...
function LiveBadge() {
    return (<div className="flex items-center gap-1 bg-red-500 rounded px-1 py-0.5 text-[10px] text-white">
